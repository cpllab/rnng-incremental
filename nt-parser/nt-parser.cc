...
struct ParserBuilder {
  ...
  explicit ParserBuilder(ParameterCollection* model, const unordered_map<unsigned, vector<float>>& pretrained) :
    ...
    // else {
      // p_t = model->add_lookup_parameters(0, {0});
      // p_t2l = model->add_parameters(0, {0});
    // }
  }
...
int main(int argc, char** argv) {
  ...
  if (conf.count("train")) {
    ...
    SimpleSGDTrainer sgd(model, 0.1*1.05);
    //AdamTrainer sgd(&model);
    //MomentumSGDTrainer sgd(&model);
    //sgd.eta_decay = 0.08;

    // sgd.eta_decay = 0.05;
    double eta_decay = 0.05;
    ...
    while(!requested_stop) {
      ...
      if (logc % 25 == 1) { // report on dev set
	   ...
	   const vector<int>& actions=dev_corpus.actions[sii];
           ...
           ComputationGraph hg;
           Expression tot_neglogprob;
           vector<unsigned> pred = parser.log_prob_parser(&hg,tot_neglogprob,sentence,vector<int>(),&right,true);
           int ti = 0;
           for (auto a : pred) {
	...
	const vector<int> actions;
        for (unsigned sii = 0; sii < test_size; ++sii) {
           ...
           for (unsigned z = 0; z < N_SAMPLES; ++z) {
             ComputationGraph hg;
             Expression tot_neglogprob;
             vector<unsigned> pred = parser.log_prob_parser(&hg,tot_neglogprob,sentence,actions,&right,sample,true);
             double lp = as_scalar(hg.incremental_forward(tot_neglogprob));
             cout << sii << " ||| " << -lp << " |||";
             int ti = 0;
             for (auto a : pred) {
